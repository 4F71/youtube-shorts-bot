...
def generate_subtitles_srt(script: Dict[str, str], total_duration: float, output_path: Path) -> Path:
    ...
    # Ensure each segment has at least 0.5s to avoid invalid SRT entries
    min_seg = 0.5
    hook_end = min(3.0, total_duration)
    story_end = max(hook_end + min_seg, total_duration - 8.0)
    story_end = min(story_end, total_duration - min_seg * 2)
    story_end = max(story_end, hook_end + min_seg)
    climax_end = max(story_end + min_seg, total_duration - 3.0)
    climax_end = min(climax_end, total_duration - min_seg)
    climax_end = max(climax_end, story_end + min_seg)

    subtitles = [
        {"index": 1, "start": 0.0, "end": hook_end, "text": hook_text},
        {"index": 2, "start": hook_end, "end": story_end, "text": story_text},
...
def apply_subtitles(video_path: Path, srt_path: Path, output_path: Path, config: Dict[str, Any]) -> Path:
    ...
    subtitle_filter = (
        ...
        f"Outline={outline_width},Alignment=2,MarginV=120,Bold=1'"
    )
...
def compose_final_video(
    ...
    try:
        ...
        logger.error("SRT generation failed — video will have NO subtitles: %s", exc)
        print(f"\nUYARI: Altyazi olusturulamadi, video altyazisiz kaydedilecek! ({exc})")
    ...
    try:
        ...
        logger.error("Subtitle apply failed — video will have NO subtitles: %s", exc)
        print(f"\nUYARI: Altyazi uygulama basarisiz, video altyazisiz kaydedilecek! ({exc})")
        shutil.copy2(temp_with_audio, output_path)
