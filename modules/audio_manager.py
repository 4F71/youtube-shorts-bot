...
def generate_tts(script_text: str, voice_id: str, output_path: Path) -> Path:
    ...
    env_voice_id = (os.getenv("ELEVENLABS_VOICE_ID") or "").strip()
    param_voice_id = (voice_id or "").strip()
    voice_to_use = param_voice_id or env_voice_id
    if not voice_to_use:
        raise ValueError("No voice_id provided and ELEVENLABS_VOICE_ID is not set")

    fallback_voice_id = env_voice_id if env_voice_id and env_voice_id != voice_to_use else None
    ...
    for attempt in range(1, retry_count + 1):
        try:
            logger.info("Generating TTS (attempt %d/%d, voice=%s)", attempt, retry_count, voice_to_use)
            ...
            if not used_fallback and fallback_voice_id:
                logger.info("Retrying with fallback voice ID: %s", fallback_voice_id)
                voice_to_use = fallback_voice_id
                used_fallback = True
                continue
